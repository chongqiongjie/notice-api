...
create table if not exists notice_tasks_summary(
task_id  int,
notice_count int,
sent_count int,
view_count int,
...
COMMENT ON TABLE public.notice_tasks_summary IS 'task级别的统计信息';
COMMENT ON COLUMN public.notice_tasks.task_id IS '任务ID';
COMMENT ON COLUMN public.notice_tasks.notice_count IS '总共需要发送的通知数目';
COMMENT ON COLUMN public.notice_tasks.sent_count IS '发送成功的数量';
COMMENT ON COLUMN public.notice_tasks.view_count IS '查看的数量';
COMMENT ON COLUMN public.notice_tasks.click_count IS '点击的数量';
...
COMMENT ON COLUMN public.notice_tasks_track_recode.track_type IS 'track url type eg:click/open ';
...
COMMENT ON COLUMN public.notice_tasks_track_recode.riid IS '所属任务的riid';
...
COMMENT ON COLUMN public.notice_tasks_track_recode.is_click IS '是否点击/查看,1/0'
COMMENT ON COLUMN public.notice_tasks_track_recode.click_time IS '点击/查看时间'

create table if not exists notice_message_template (
...
COMMENT ON TABLE public.notice_message_template IS '消息模版表';
COMMENT ON COLUMN public.notice_message_template.tid IS '主键自增ID'
COMMENT ON COLUMN public.notice_message_template.temp_name IS '模版名称'
COMMENT ON COLUMN public.notice_message_template.message_type IS '消息类型sms/email'
COMMENT ON COLUMN public.notice_message_template.temp_type IS '模版类型，public／private'
COMMENT ON COLUMN public.notice_message_template.user_id IS '所属用户'
COMMENT ON COLUMN public.notice_message_template.template_subject IS '消息的头部，用于email'
COMMENT ON COLUMN public.notice_message_template.template_content IS '消息的内容，用于sms 和email'
COMMENT ON COLUMN public.notice_message_template.create_time IS '创建时间'
COMMENT ON COLUMN public.notice_message_template.update_time IS '修改时间'
COMMENT ON COLUMN public.notice_message_template.is_valid IS '状态0/1'
